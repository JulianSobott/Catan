...
public class LobbyBackground extends Background {
	...
	String fragmentShaderBackground = "#ifdef GL_ES\n"
			...
			+ "  if(dist < 50){"
			+ "   val = 1.0;"
			+ "    gl_FragColor = vec4(1f, 0.7f, 0, val);}"
			+ "  else{"
			+ "  val = 500/dist; \n"
			+ "  val = min(val, 0.7); \n"
			+ "  val = max(val, 0.0);"
			+ "  vec4 brightness = vec4(1f,1f,1f,1f);"
			+ "  if(sunPosition.y < 100){"
			+ "    brightness = vec4(1f,1f,1f,1f);"
			+ "  }"
			+ "  gl_FragColor = vec4(226f/255f, 109f/255f, 13f/255f, val) * brightness;} \n"			
